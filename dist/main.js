...
function compileMap() {
    ...
    function collectType(type1) {
        if (type1.multi) {
            ...
            result[type1.kind][type1.tag] = result['fallback'][type1.tag] = type1;
        }
    }
...
Schema$1.prototype.extend = function extend1(definition) {
    var implicit = [];
    var explicit = [];
...
function resolveYamlBinary(data) {
    if (data === null) return false;
    var code, idx, bitlen = 0, max = data.length, map1 = BASE64_MAP;
...
function constructYamlBinary(data) {
    var idx, tailbits, input = data.replace(/[\r\n=]/g, ''), max = input.length, map1 = BASE64_MAP, bits = 0, result = [];
    for(idx = 0; idx < max; idx++){
        if (idx % 4 === 0 && idx) {
        ...
        bits = bits << 6 | map1.indexOf(input.charAt(idx));
    }
    tailbits = max % 4 * 6;
    if (tailbits === 0) {
        result.push(bits >> 16 & 255);
...
function representYamlBinary(object) {
    var result = '', bits = 0, idx, tail, max = object.length, map1 = BASE64_MAP;
    for(idx = 0; idx < max; idx++){
        if (idx % 3 === 0 && idx) {
            result += map1[bits >> 18 & 63];
            result += map1[bits >> 12 & 63];
            result += map1[bits >> 6 & 63];
            result += map1[bits & 63];
        }
    ...
    if (tail === 0) {
        ...
        result += map1[bits >> 10 & 63];
        result += map1[bits >> 4 & 63];
        result += map1[bits << 2 & 63];
        result += map1[64];
    } else if (tail === 1) {
        result += map1[bits >> 2 & 63];
        result += map1[bits << 4 & 63];
...
function composeNode(state, parentIndent, nodeContext, allowToSeek, allowCompact) {
    var allowBlockStyles, allowBlockScalars, allowBlockCollections, indentStatus = 1, atNewLine = false, hasContent = false, typeIndex, typeQuantity, typeList, type1, flowIndent, blockIndent;
    ...
    if (state.tag === null) {
        ...
        if (state.result !== null && type1.kind !== state.kind) {
            throwError(state, 'unacceptable node kind for !<' + state.tag + '> tag; it should be "' + type1.kind + '", not "' + state.kind + '"');
        }
        if (!type1.resolve(state.result, state.tag)) {
            throwError(state, 'cannot resolve a node with !<' + state.tag + '> explicit tag');
        } else {
            state.result = type1.construct(state.result, state.tag);
            if (state.anchor !== null) {
                state.anchorMap[state.anchor] = state.result;
            }
        }
...
function compileStyleMap(schema1, map1) {
    var result, keys, index, length, tag, style, type1;
    if (map1 === null) return {
    };
    result = {
    };
    ...
    for(index = 0, length = keys.length; index < length; index += 1){
        tag = keys[index];
        style = String(map1[tag]);
        if (tag.slice(0, 2) === '!!') {
            tag = 'tag:yaml.org,2002:' + tag.slice(2);
        }
        type1 = schema1.compiledTypeMap['fallback'][tag];
        if (type1 && _hasOwnProperty.call(type1.styleAliases, style)) {
...
function writeScalar(state, string, level, iskey, inblock) {
    state.dump = (function() {
        ...
        function testAmbiguity(string1) {
            return testImplicitResolving(state, string1);
        }
...
function escapeString(string) {
    ...
    for(var i1 = 0; i1 < string.length; __char >= 65536 ? i1 += 2 : i1++){
        ...
        if (!escapeSeq && isPrintable(__char)) {
            result += string[i1];
            if (__char >= 65536) result += string[i1 + 1];
        } else {
...
function detectType(state, object, explicit) {
    var _result, typeList, index, length, type1, style;
    ...
    for(index = 0, length = typeList.length; index < length; index += 1){
        ...
        if ((type1.instanceOf || type1.predicate) && (!type1.instanceOf || typeof object === 'object' && object instanceof type1.instanceOf) && (!type1.predicate || type1.predicate(object))) {
            if (explicit) {
                if (type1.multi && type1.representName) {
                    state.tag = type1.representName(object);
                } else {
                    state.tag = type1.tag;
                }
            } else {
            ...
            if (type1.represent) {
                ...
                if (_toString.call(type1.represent) === '[object Function]') {
                    _result = type1.represent(object, style);
                } else if (_hasOwnProperty.call(type1.represent, style)) {
                    _result = type1.represent[style](object, style);
                } else {
                    throw new exception('!<' + type1.tag + '> tag resolver accepts not "' + style + '" style');
                }
                state.dump = _result;
            }
            return true;
...
function writeNode(state, level, object, block, compact, iskey, isblockseq) {
    ...
    var type1 = _toString.call(state.dump);
    var inblock = block;
    var tagStr;
    if (block) {
        block = state.flowLevel < 0 || state.flowLevel > level;
    }
    var objectOrArray = type1 === '[object Object]' || type1 === '[object Array]', duplicateIndex, duplicate;
    if (objectOrArray) {
    ...
    if (duplicate && state.usedDuplicates[duplicateIndex]) {
        ...
        } else if (type1 === '[object Undefined]') {
            return false;
        } else {
            if (state.skipInvalid) return false;
            throw new exception('unacceptable kind of an object to dump ' + type1);
        }
